...
def test_BasicSimpleSentence():
    response = client.post("/analyzeOneSentence",
                        headers={"Content-Type": "application/json"},
                        json={"sentence": "This is a simple test.", "extentInfoJson": "{}"})    
    assert response.status_code == 200
    aso = AnalyzedSentenceObject.parse_obj(response.json())
...
def test_NegativeSimpleSentence():
    response = client.post("/analyzeOneSentence",
                            headers={"Content-Type": "application/json"},
                            json={"sentence": "The problem does not seem soluble.", "extentInfoJson": "{}"})    
    assert response.status_code == 200
    aso = AnalyzedSentenceObject.parse_obj(response.json())
    scoresSorted = sorted(aso.nodeMap.values(), key=lambda x:x.currentId)
    surfaces = []
...
def test_SimpleSentenceWithConditionalClauses():
    response = client.post("/analyzeOneSentence",
                        headers={"Content-Type": "application/json"},
                        json={"sentence": "If you heat ice, it melts.", "extentInfoJson": "{}"})    
    assert response.status_code == 200
    aso = AnalyzedSentenceObject.parse_obj(response.json())
    scoresSorted = sorted(aso.nodeMap.values(), key=lambda x:x.currentId)
    surfaces = []
...
def test_SimpleSentenceWithConditionalClauses3():
    response = client.post("/analyzeOneSentence",
                        headers={"Content-Type": "application/json"},
                        json={"sentence": "I didn't bring an umbrella, as the wind is so strong today.", "extentInfoJson": "{}"})    
    assert response.status_code == 200
    aso = AnalyzedSentenceObject.parse_obj(response.json())
    scoresSorted = sorted(aso.nodeMap.values(), key=lambda x:x.currentId)
    surfaces = []
...
def test_SimpleSentenceWithQuantitativeExpressions():
    response = client.post("/analyzeOneSentence",
                        headers={"Content-Type": "application/json"},
                        json={"sentence": "His weight is over 70kg.", "extentInfoJson": "{}"})    
    assert response.status_code == 200
    aso = AnalyzedSentenceObject.parse_obj(response.json())
    scoresSorted = sorted(aso.nodeMap.values(), key=lambda x:x.currentId)
    surfaces = []
...
def test_SimpleSentenceWithQuantitativeExpressions2():
    response = client.post("/analyzeOneSentence",
                        headers={"Content-Type": "application/json"},
                        json={"sentence": "Its stock price has risen by more than $ 10.", "extentInfoJson": "{}"})    
    assert response.status_code == 200
    aso = AnalyzedSentenceObject.parse_obj(response.json())
    scoresSorted = sorted(aso.nodeMap.values(), key=lambda x:x.currentId)
    surfaces = []
...
def test_IrregularSimpleSentence():
    try:
        response = client.post("/analyzeOneSentence",
                            headers={"Content-Type": "application/json"},
                            json={"sentence": "!#$%&Y'\"UIO\n strange =*+<H`OJWKFHgb", "extentInfoJson": "{}"})    
        assert response.status_code == 200
        aso = AnalyzedSentenceObject.parse_obj(response.json())        
    except Exception:
        pytest.fail("Unexpected Error ..")
...
def test_PremiseOneSentenceAndClaimEmpty():
    response = client.post("/analyze",
                        headers={"Content-Type": "application/json"},
                        json={"premise": ["The answer is blown'in the wind."], "claim": []})    
    assert response.status_code == 200
    asos = AnalyzedSentenceObjects.parse_obj(response.json())
...
def test_PremiseEnmptyAndClaimOneSentence():
    response = client.post("/analyze",
                        headers={"Content-Type": "application/json"},
                        json={"premise": [], "claim": ["The answer is blown'in the wind."]})    
    assert response.status_code == 200
    asos = AnalyzedSentenceObjects.parse_obj(response.json())
    assert len(asos.analyzedSentenceObjects) == 1
    aso = asos.analyzedSentenceObjects[0]
...
def test_PremiseOneSentencetyAndClaimOneSentence():
    response = client.post("/analyze",
                        headers={"Content-Type": "application/json"},
                        json={"premise": ["You may say I'm a dreamer, But I'm not the only one."], "claim": ["I hope someday you'll join us And the world will live as one."]})    
    assert response.status_code == 200
    asos = AnalyzedSentenceObjects.parse_obj(response.json())
...
def test_PremiseMultipleSentencetyAndClaimMultipleSentence():
    response = client.post("/analyze",
                        headers={"Content-Type": "application/json"},
                        json={"premise": ["Just The Way You Are!", "The answer is blown'in the wind."], "claim": ["You may say I'm a dreamer, But I'm not the only one.", "I hope someday you'll join us And the world will live as one."]})    
    assert response.status_code == 200
    asos = AnalyzedSentenceObjects.parse_obj(response.json())
